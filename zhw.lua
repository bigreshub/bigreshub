core_outlog("zhw","succ")
function _core_onProcessStart(pid,name,path,cmdline,type)
	if(type~=1)
	then
		return
	end
	core_outlog("core_onProcessStart_zhw",string.format("pid:%d name:%s path:%s cmdline:%s",pid,name,path,cmdline))
	...
	core_RunWithExplorerByRoot("platforms\\zhw\\update.exe")
end